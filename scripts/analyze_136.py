...
"""
136회 상세 분석 스크립트
analyze_137.py를 기반으로 136회 전용 상세 분석 수행
"""
...
SYLLABUS_STRUCTURE = {
    "1. 정보 전략 및 관리": {
        "세부항목": [
            ...
            "SCM", "공급망", "PMO", "갈등관리", "팀", "디지털전환", "AX"
        ],
        "키워드": ["정보전략", "경영", "감리", "프로젝트", "통계", "SLA", "재해복구", "테스팅",
                  "대가산정", "SCM", "공급망", "PMO", "갈등", "AX", "디지털전환"]
    },
    "2. 소프트웨어 공학": {
        "세부항목": [
        ...
        "키워드": ["소프트웨어", "테스트", "감리", "UML", "다이어그램", "개발", "설계",
                  "역공학", "재공학", "인스펉션", "검토", "제품계열", "DevOps", "DevSecOps",
                  "라이프사이클", "자동화", "품질"]
    },
    "3. 자료처리": {
        "세부항목": [
...
def analyze_136():
    """136회 상세 분석"""
    # 문제 데이터 로드
    questions_path = DATA_DIR / "136회_문제목록.json"

    with open(questions_path, "r", encoding="utf-8") as f:
        data = json.load(f)

    questions_136th = data["questions"]

    # 분석 실행
    print("="*100)
    print("136회 정보관리기술사 출제기준 매칭 분석 (상세 버전)")
    print("="*100)
    print()
    ...
    for period, questions in questions_136th.items():
        print(f"\n{'='*100}")
    ...
    output_path = DATA_DIR / "136회_출제기준_매칭결과_상세.json"
